...
它不是语音输入，也不会一次生成一大段文字。它的目标很窄：观察你经常打错的拼音，在你改正后学习这条习惯，下次输入错误拼音时让小狼毫优先给出正确候选词。
...
3. 在“输入法”页面确认 Rime 用户目录和方案 ID，点击“部署纠错词典”。
4. 从小狼毫菜单执行一次“重新部署”。
...
这个文件用来保存模型地址和 API Key。你不需要先手动编辑它；在设置界面保存模型配置后，程序会自动写入。
...
纠错词典只应在完整错误拼音输入后生效。下面这些短前缀不应把“现在”提前到候选前列：
...
AI IME 默认会写入雾凇拼音方案文件，并生成这些文件：

```text
rime_ice.custom.yaml
ai_typo.dict.yaml
ai_typo.schema.yaml
lua/ai_ime_logger.lua
.ai-ime-backups/
```

其中 .ai-ime-backups 目录用于备份被改动的 Rime 文件。
